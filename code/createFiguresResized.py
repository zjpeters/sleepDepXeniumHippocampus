...
regionalColorListDict = {'CA1': ca1Color, 'CA2': ca2Color, 'CA3': ca3Color, 'DG': dgColor, 'DG/CA4': ca4dgColor}
...
for i in sampleOrder:
    ...
    ax[nR, nC].axis('off')
    # ax[nR, nC].set_title(f"{sdStatus}-{sample['sampleID']}")
    # use SD-1, NSD-1 as labels, rather than original sample names
    ax[nR, nC].set_title(f"{sdStatus}-{nC + 1}")
    if nC < 2:
        nC += 1
    else:
        nR += 1
...
for regionN, region in enumerate(cellsOfInterest):
    for gene in range(len(sampleForDisplay['geneList'])):
        if bhCorrList[gene, regionN] == 1 and np.log2(foldChangeList[gene,regionN]) < 0:
            sigColorList[gene, regionN] = -1# '#1f77b4'
        elif bhCorrList[gene, regionN] == 1 and np.log2(foldChangeList[gene,regionN]) > 0:
            sigColorList[gene, regionN] = 1# '#FF0000'
        else:
...
degPerRegion = []
for i in cellsOfInterest[0:10]:
    if i == 'DG/CA4':
        degPerRegion.append(len(degDict['DG-CA4']))
    else:
        degPerRegion.append(len(degDict[i]))
degPerRegionNoSparse = np.array(degPerRegion)
...
ca1Plot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
ca2Plot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
ca3Plot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
dgPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
#%% make volcano plot with hippocampus image for display
...
hippPlot.set_xticks([])
hippPlot.set_yticks([])
hippPlot.spines['top'].set_visible(False)
hippPlot.spines['bottom'].set_visible(False)
hippPlot.spines['left'].set_visible(False)
hippPlot.spines['right'].set_visible(False)
hippPlot.annotate('CA1 (89)', [740, 25])
hippPlot.annotate('CA2 (34)', [20, 200])
...
for j in enumerate(sortedPval[:nOfGenesToPlot]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        a = ca1Plot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])), fontsize=volcanoGeneFontSize)
        a.draggable()
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[0,0].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
...
for j in enumerate(sortedPval[:nOfGenesToPlot]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        a = ca2Plot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])), fontsize=volcanoGeneFontSize)
        a.draggable()
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[0,0].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
...
for j in enumerate(sortedPval[:nOfGenesToPlot]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        a = ca3Plot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])), fontsize=volcanoGeneFontSize)
        a.draggable()
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[0,0].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
...
for j in enumerate(sortedPval[:nOfGenesToPlot]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        a = dgPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])), fontsize=volcanoGeneFontSize)
        a.draggable()
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[0,0].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
...
### edited locations of gene annotations to prevent overlap in inkscape, so only generate new if necessary
# output pdf and svg
plt.savefig(os.path.join(figureFolder, f'figure02_regional_volcano_plots_volcano_marker_{volcanoMarkerSize}_with_hipp.pdf'), bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(figureFolder, f'figure02_regional_volcano_plots_volcano_marker_{volcanoMarkerSize}_with_hipp.svg'), bbox_inches='tight', dpi=300)
plt.close('all')
...
barPlot.grid(zorder=0, axis='y')
barPlot.bar(cellsOfInterestNoSparse[reorderMask], degPerRegionNoSparse[reorderMask], color=cellsOfInterestColorListNoSparse[reorderMask], zorder=3)
barPlot.set_ylim(top=barMax)
barPlot.set_ylabel('Number of DEGs', fontweight='bold')
barPlot.set_yticks(np.arange(0,barMax+0.1, barMax/4))
...
neuPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
for j in enumerate(sortedPval[:10]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        a = neuPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])))
        a.draggable()
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[0,0].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
...
dgIntPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
for j in enumerate(sortedPval[:10]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        a = dgIntPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])))
        a.draggable()
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
...
astPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
for j in enumerate(sortedPval[:10]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        astPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])))
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
...
micPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
for j in enumerate(sortedPval[:10]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        micPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])))
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
...
oliPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
for j in enumerate(sortedPval[:10]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        oliPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])))
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[1,1].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
if cellsOfInterest[cellTypeIdx] == 'DG/CA4':
...
endPlot.scatter(np.log2(foldChangeList[:,cellTypeIdx]), -np.log10(pValList[:,cellTypeIdx]), c=colorArray, s=volcanoMarkerSize, linewidth=0)
...
for j in enumerate(sortedPval[:10]):
    if bhCorrList[j[1],cellTypeIdx] == 1:
        endPlot.annotate(sampleForDisplay["geneList"][j[1]], (np.log2(foldChangeList[j[1],cellTypeIdx]), -np.log10(pValList[j[1],cellTypeIdx])))
# for j in enumerate(sigColorList[:,cellTypeIdx]):
#     if j[1] != 0:
#         ax[1,1].annotate(sampleForDisplay["geneList"][j[0]], (logFCList[j[0],cellTypeIdx], -np.log10(pValList[j[0],cellTypeIdx])))
if cellsOfInterest[cellTypeIdx] == 'DG/CA4':
...
"""
UpSet plot with externally updated colors will be saved as .png and loaded for 
use in figure 4
"""
...
def plotGeneAndTStatToSubplots(processedSample, nsdMeanFig, sdMeanFig, tStatFig, 
                               geneName, cellTypePriority=None, marker_size=0.2,
                               displayMax=None):
    geneIdx = processedSample['geneList'].index(geneName)
    cellTypeList = cellsOfInterest[0:10]
    labeledCellsIdx = np.where(allClustersMale != 'sparse')[0]
    # if cellTypePriority == None:
    maxExp = np.max(allCellsMale[geneIdx, labeledCellsIdx])
    # else:
    #     cellTypeIdx = np.where(allClustersMale == cellTypePriority)[0]
    #     maxExp = np.max(labeledCells[geneIdx, cellTypeIdx])
    #     print(np.mean(allCellsMale[geneIdx, cellTypeIdx] > 0), maxExp)
    #     cellTypeList = cellsOfInterest[0:10]
        # regionIdx = cellTypeList.index(cellTypePriority)
        # cellTypeList.insert(len(cellTypeList)+1, cellTypeList.pop(regionIdx))
    if displayMax != None:
        maxExp = displayMax
    # orderList = []
    # for i in cellTypeList:
    #     orderList.append(cellsOfInterest[0:10].index(i))
    # sigArray = np.squeeze(np.array(bhCorrList[geneIdx, 0:10]))
    
    nsdMeanFig.imshow(processedSample['tissueImageProcessed'], cmap='gray_r')
    sdMeanFig.imshow(processedSample['tissueImageProcessed'], cmap='gray_r')
    tStatFig.imshow(processedSample['tissueImageProcessed'], cmap='gray_r')
    
    for regionN, region in enumerate(cellTypeList):
        if cellTypePriority != None:
            alphaLevel = 0.3
        else:
            alphaLevel = 1
        regionCellsToPlot = findRelevantClusters(processedSample, region)
        regionIdx = np.where(allClustersMale == region)[0]
        sdNSDRegionIdx = sdNSDIdxMale[regionIdx]
        regionCells = allCellsMale[:, regionIdx]
        nsdCells = regionCells[:, sdNSDRegionIdx == 0]
        ...
        nsdColor = np.empty([len(regionCellsToPlot)])
        nsdCells = np.squeeze(np.array(nsdCells[geneIdx,:]))
        nsdColor[:] = np.mean(nsdCells[nsdCells > 0])
        # print(np.mean(nsdCells[geneIdx,:] > 0))
        sdColor = np.empty([len(regionCellsToPlot)])
        sdCells = np.squeeze(np.array(sdCells[geneIdx,:]))
        sdColor[:] = np.mean(sdCells[sdCells > 0])
        # print(np.mean(sdCells[geneIdx,:] > 0))
        tStatColor = np.empty([len(regionCellsToPlot)])
        tStatColor[:] = tStat
        nsdMeanFig.scatter(processedSample['processedTissuePositionList'][regionCellsToPlot,0], processedSample['processedTissuePositionList'][regionCellsToPlot,1], c=nsdColor, cmap='Reds', s=marker_size, linewidth=0, vmin=0, vmax=maxExp)
        nsdMeanFig.set_title('Mean of NSD', fontweight='bold')
        nsdMeanFig.set_xticks([])
        nsdMeanFig.set_yticks([])
        
        meanScatter = sdMeanFig.scatter(processedSample['processedTissuePositionList'][regionCellsToPlot,0], processedSample['processedTissuePositionList'][regionCellsToPlot,1], c=sdColor, cmap='Reds', s=marker_size, linewidth=0, vmin=0, vmax=maxExp)
        sdMeanFig.set_title('Mean of SD', fontweight='bold')
        sdMeanFig.set_xticks([])
        sdMeanFig.set_yticks([])
        
        tstatScatter = tStatFig.scatter(processedSample['processedTissuePositionList'][regionCellsToPlot,0], processedSample['processedTissuePositionList'][regionCellsToPlot,1], c=tStatColor, cmap='seismic', s=marker_size, linewidth=0, vmin=-4, vmax=4, alpha=alphaLevel)
        tStatFig.set_title('T-statistic', fontweight='bold')
        tStatFig.set_xticks([])
        tStatFig.set_yticks([])
    # add overlay of cellTypePriority if included
    if cellTypePriority != None:
        alphaLevel = 1
        region = cellTypePriority
        regionN = cellTypeList.index(region)
        regionCellsToPlot = findRelevantClusters(processedSample, region)
        regionIdx = np.where(allClustersMale == region)[0]
        sdNSDRegionIdx = sdNSDIdxMale[regionIdx]
        regionCells = allCellsMale[:, regionIdx]
    ...
    nsdMeanFig.set_ylabel(geneName, style='italic')
    mCbar = plt.colorbar(meanScatter, ax=[nsdMeanFig, sdMeanFig], orientation='horizontal', fraction=0.05, pad=0.04)
...
# DG
gene06 = 'Egr3'
plotGeneAndTStatToSubplots(sampleForDisplay, gene01NSDMean, gene01SDMean, gene01Tstat, 'Cirbp')
gene01NSDMean.text(-30, -30, 'B', fontweight='bold', size=9)
plotGeneAndTStatToSubplots(sampleForDisplay, gene02NSDMean, gene02SDMean, gene02Tstat, 'Rbm3')
gene02NSDMean.text(-30, -30, 'C', fontweight='bold', size=9)
plotGeneAndTStatToSubplots(sampleForDisplay, gene03NSDMean, gene03SDMean, gene03Tstat, 'Gpr161', cellTypePriority='CA1')
gene03NSDMean.text(-30, -30, 'D', fontweight='bold', size=9)
plotGeneAndTStatToSubplots(sampleForDisplay, gene04NSDMean, gene04SDMean, gene04Tstat, 'Jdp2', cellTypePriority='CA3')
gene04NSDMean.text(-30, -30, 'E', fontweight='bold', size=9)
plotGeneAndTStatToSubplots(sampleForDisplay, gene05NSDMean, gene05SDMean, gene05Tstat, 'Egr3', cellTypePriority='DG')
gene05NSDMean.text(-30, -30, 'F', fontweight='bold', size=9)
plotGeneAndTStatToSubplots(sampleForDisplay, gene06NSDMean, gene06SDMean, gene06Tstat, 'Dcn', cellTypePriority='astrocytes')
...
for cellType in enumerate(uniqueDegs):
    ...
    for gene in uniqueGeneList:
        ...
        # for subplot2grid: size of plot (rows, columns) plot location (rows, columns) 
        NSDMean = plt.subplot2grid((6,9), (0,0), colspan=3, rowspan=2)
...
#%%  recreate the figures above, but sorted based on cluster region ID
...
for sampleIdx in range(len(maleSamples)):
    ...
    if experiment['experimental-group'][sampleIdx] == 0:
        sdStatus = f'NSD-{nsdN}'
        nsdN += 1
    elif experiment['experimental-group'][sampleIdx] == 1:
        sdStatus = f'SD-{sdN}'
        sdN += 1
    nOfClusters = 0
    for tempClust in sampleForDisplay['cluster_region']:
        if tempClust in regionalClusters:
            nOfClusters += 1
    # plt.close('all')
    fig,ax = plt.subplots(len(regionalGenes)+1, nOfClusters, figsize=(7.2,3.6))
    ...
    for clusterName in regionalClusters:
        ...
        for i in regionIdx:
            ...
            for rowIdx, gene in enumerate(regionalGenes):
                ...
                scatterAx = ax[0,columnIdx].scatter(sampleForDisplay['processedTissuePositionList'][clusterIdx,0], sampleForDisplay['processedTissuePositionList'][clusterIdx,1],c=clusterColorArray, s=0.3, linewidth=0)
                ax[0, 0].set_ylabel('Assigned cluster', rotation='horizontal', horizontalalignment='right', fontsize=5, fontweight='bold')
                geneIdx = sampleForDisplay['geneList'].index(gene)
                ...
                ax[0, columnIdx].set_title(clusterTitle, fontsize=5, fontweight='bold')
                ax[rowIdx+1, 0].set_ylabel(gene, rotation='horizontal', horizontalalignment='right', fontsize=5, fontweight='bold', style='italic')
                ax[rowIdx+1,columnIdx].imshow(sampleForDisplay['tissueImageProcessed'],cmap='gray_r')
                scatterAx = ax[rowIdx+1, columnIdx].scatter(sampleForDisplay['processedTissuePositionList'][clusterIdx,0], sampleForDisplay['processedTissuePositionList'][clusterIdx,1],c=cellTypeMatrixMeanZScore, cmap='Reds', s=0.3, vmin=0, vmax=2, linewidth=0)
                ax[rowIdx+1,columnIdx].tick_params(
                    ...
                    labelleft=False)
            columnIdx += 1
    plt.suptitle(f'Mean z-score for regional marker genes in {sdStatus}', fontsize=7, fontweight='bold')
    fig.text(0.5, 0.04, 'Assigned cluster', ha='center', fontsize=5, fontweight='bold')
    fig.text(0.04, 0.5, 'Gene marker', va='center', rotation='vertical', fontsize=5, fontweight='bold')
...
fullCellTypeNames = {'ast': 'astrocytes', 'end': 'endothelial', 'mic': 'microglia', 'oli': 'oligodendrocytes', 'interneurons': 'interneurons'}
# plt.close('all')
nsdN = 1
sdN = 1
for sampleIdx in range(len(maleSamples)):
    ...
    fig,ax = plt.subplots(len(cellTypeGeneLists), nOfClusters, figsize=(7.2,3.6))
        ...
        for i in regionIdx:
            ...
            for rowIdx, cellType in enumerate(cellTypeGeneLists): 
                if cellType == 'neu':
                    ...
                    # ax[0, i].set_title(f'Cluster {i}')
                    ax[0, columnIdx].set_title(clusterTitle, fontsize=5, fontweight='bold')
                    ax[rowIdx, 0].set_ylabel(fullCellTypeNames[cellType], rotation='horizontal', horizontalalignment='right', fontsize=5, fontweight='bold')
                    ax[rowIdx,columnIdx].imshow(sampleForDisplay['tissueImageProcessed'],cmap='gray_r')
                    scatterAx = ax[rowIdx,columnIdx].scatter(sampleForDisplay['processedTissuePositionList'][clusterIdx,0], sampleForDisplay['processedTissuePositionList'][clusterIdx,1],c=cellTypeMatrixMeanZScore, cmap='Reds', s=0.3, vmin=0, vmax=2, linewidth=0)
    ...
    plt.suptitle(f'Mean z-score for cell type marker genes, {sdStatus}', fontsize=7, fontweight='bold')
    fig.text(0.5, 0.04, 'Assigned cluster', ha='center', fontsize=6, fontweight='bold')
    fig.text(0.04, 0.5, 'Marker genes for cell type', va='center', rotation='vertical', fontsize=6, fontweight='bold')
    cbar_ax = fig.add_axes([0.92, 0.13, 0.01, 0.6])
    fig.colorbar(scatterAx, cax=cbar_ax, fraction=0.01, pad=0.04)
    plt.show()
