...
for actSample in range(len(maleSamples)):
    ...
    if designMatrix[actSample] == 0:
        sdNSDIdx = np.append(sdNSDIdx, np.zeros(len(maleSamples[actSample]['processedTissuePositionList'])).T)
    elif designMatrix[actSample] == 1:
        sdNSDIdx = np.append(sdNSDIdx, np.ones(len(maleSamples[actSample]['processedTissuePositionList'])).T)
...
plt.savefig(os.path.join(derivatives, 'xenium_figure_k15cluster_umap_with_sd_nsd_split.svg'), bbox_inches='tight', dpi=300)
