...
for actSample in range(len(maleSamples)):
    ...
    for cluster in maleSamples[actSample]['cluster_labels'].to_numpy():
        clusterList.append(maleSamples[actSample]['cluster_region'][cluster])
    allClusters = np.append(allClusters, clusterList)
    # for i in enumerate(maleSamples[actSample]['cluster_region']):
    #     x = np.where(maleSamples[actSample]['cluster_labels'].to_numpy() == i[0])
    #     clusterList[x[0]] = i[1]
    #     print(i[1], x)
    # binary distinction of each cell for sd or nsd
    if designMatrix[actSample] == 0:
        sdNSDIdx = np.append(sdNSDIdx, np.zeros(len(maleSamples[actSample]['processedTissuePositionList']), dtype='int32').T)
    elif designMatrix[actSample] == 1:
        sdNSDIdx = np.append(sdNSDIdx, np.ones(len(maleSamples[actSample]['processedTissuePositionList']), dtype='int32').T)
    sampleColors = maleSamples[actSample]['cluster_colors']
clusterList = np.array(clusterList, dtype='str')
...
#%% create a plot of umap for SD, umap for NSD, and UMAP for all cells
...
nsdIdx = np.where(sdNSDIdx == 0)[0]
umapAxsNSD.scatter(embedding[nsdIdx,0], embedding[nsdIdx,1], c=colorsAllCells[nsdIdx,:], s=3)
umapAxsNSD.set_title("UMAP of NSD samples")
umapAxsNSD.set_xticks([])
umapAxsNSD.set_yticks([])

# plot the SD samples
sdIdx = np.where(sdNSDIdx == 1)[0]
umapAxsSD.scatter(embedding[sdIdx,0], embedding[sdIdx,1], c=colorsAllCells[sdIdx,:], s=3)
umapAxsSD.set_title("UMAP of SD samples")
...
# plot all cells
umapAxsAllCells.scatter(embedding[:,0], embedding[:,1], c=colorsAllCells, s=3)
umapAxsAllCells.set_title("UMAP of all samples")
umapAxsAllCells.set_xticks([])
umapAxsAllCells.set_yticks([])

plt.show()

plt.savefig(os.path.join(derivatives, 'xenium_figure_k15cluster_umap_with_sd_nsd_split.pdf'), bbox_inches='tight', dpi=300)
#%% try plotting above data but with markers swapped between conditions

plt.close('all')
# plot all cells
fig, umapAxsAllCells = plt.subplots(1, 1)
umapAxsAllCells.scatter(embedding[nsdIdx,0], embedding[nsdIdx,1], c=colorsAllCells[nsdIdx,:], marker='x', s=2)
umapAxsAllCells.set_title("UMAP of all samples, NSD='x', SD='+'")
umapAxsAllCells.set_xticks([])
umapAxsAllCells.set_yticks([])

umapAxsAllCells.scatter(embedding[sdIdx,0], embedding[sdIdx,1], c=colorsAllCells[sdIdx,:], marker='+', s=2)
umapAxsAllCells.set_xticks([])
umapAxsAllCells.set_yticks([])
plt.show()
plt.savefig(os.path.join(derivatives, 'xenium_figure_k15cluster_umap_with_sd_nsd_marked.svg'), bbox_inches='tight', dpi=300)
...
for region in cellsOfInterest:
    ...
    regionIdx = np.where(allClusters == region)[0]
    sdNSDRegionIdx = sdNSDIdx[regionIdx]
    regionCells = allCells[:, regionIdx]
    ...
    for gene in range(len(sampleToCluster['geneList'])):
        ...
        if pVal < alphaFdr:
            ...
            ax[2].axis('off')
            plt.suptitle(f'T-statistic for gene: {sampleToCluster["geneList"][gene]} \n in {region}')
            plt.show()
            if region == "DG/CA4":
                plt.savefig(os.path.join(derivatives, 'clusterDEGs', f'ttest_nsd_sd_{sampleToCluster["geneList"][gene]}_in_DG-CA4.png'), bbox_inches='tight', dpi=300)
            else:
                plt.savefig(os.path.join(derivatives, 'clusterDEGs', f'ttest_nsd_sd_{sampleToCluster["geneList"][gene]}_in_{region}.png'), bbox_inches='tight', dpi=300)
            plt.close()
            sigGenesPerCells[region].append(sampleToCluster["geneList"][gene])
...
for gene in range(len(sampleToCluster['geneList'])):
    ...
    """
    plt.savefig(os.path.join(derivatives, 'clusterDEGsAllRegions', f'ttest_nsd_sd_{sampleToCluster["geneList"][gene]}_in_allRegions.png'), bbox_inches='tight', dpi=300)
    plt.close()
